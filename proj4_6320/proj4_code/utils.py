...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
  ...
  ############################################################################
  # 1. Generate the left image to be random 0/1 values
  im_left = torch.randint(low=0, high=2, size = im_size)
  # 2. Set the right image as the copy of the left image
  ...
  bw = W // 2
  block_top = (H-bh) // 2
  block_bot = (block_top + bh)

  block_left = (W - bw) // 2
  block_right = (block_left + bw)

  # block = im_left[block_top:block_bot, block_left:block_right, :]
  im_right[block_top:block_bot, block_left - disparity: block_right - disparity, :] = im_right[block_top:block_bot, block_left:block_right,:]
  im_right[block_top:block_bot, block_right-disparity: block_right, :] = torch.randint(low=0, high=2, size = (bh, disparity, C)) 
